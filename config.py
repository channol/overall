...
uemgr_file = dirpath+'/uemgr'+strftime+'.log'
...
def get_log():
    ...
    os.popen('sudo dcomp logs uemgr > {}'.format(uemgr_file))
    ...
    if os.path.exists(uemgr_file):
        logging.info('scp root@172.0.5.27:'+uemgr_file+' .')
        logging.info('vim scp://root@172.0.5.27/'+uemgr_file)
    else:
        logging.warning('The uemgr log is not exist!')
